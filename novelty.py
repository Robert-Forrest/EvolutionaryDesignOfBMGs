...
def novelty(alloys):
    ...
    model = cb.models.load(
        "/home/rob/phd/code/GFAPaper/Tl_Tg_Tx_deltaT_GFA_Dmax/ensemble"
    )
    data = cb.features.load_data(
        datafiles=["~/phd/code/GFAPaper/data/data.csv"],
        model=model,
    ...
    predict_alloy_data, targets, input_features = cb.features.calculate_features(
        alloys["alloy"],
    ...
    alloys["novelty"] = -lof.decision_function(
